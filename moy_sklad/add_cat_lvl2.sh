...
UUIDS=$($MYSQL_CMD "SELECT uuid FROM msklad_cat;")

for i in ${UUIDS[@]}; do
    $MYSQL_CMD "SELECT t2.uuid, t1.description FROM categories_level2 as t1 LEFT JOIN msklad_cat as t2 ON t2.ml = t1.root_cat WHERE t2.uuid = '${i}';" > $i
