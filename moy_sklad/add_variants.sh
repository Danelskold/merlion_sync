...
for i in ${ITEMS[@]}; do
	...
	for p in ${PROPERTIES[@]}; do
	    $MYSQL_CMD "SELECT DISTINCT t1.uuid,';',t1.name,';',t2.property_name FROM msklad_characteristics AS t1 RIGHT JOIN \
	        items_properties as t2 ON t2.property_id = t1.ml WHERE t1.ml = '${p}' AND t2.no = '${i}';"  >> /tmp/${FILE_PART}_${i}
	done
