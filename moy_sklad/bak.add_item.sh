...
for i in ${ITEMS[@]}; do
	...
	PRODUCT_NAME=$($MYSQL_CMD "SELECT Name from items WHERE no = '${i}'" | sed -e 's/\"/\\\\"/g' -e 's/\//\\\//g' -e 's/(/\\\\(/g' \
            -e 's/)/\\\\)/g' -e 's/\./\\\\./g')
	echo $PRODUCT_NAME
	...
	LINE=$(( $NUM + 1 ))
		...
		VARIANT_UUID=$(cat /tmp/${FILE_PART}_${i} | sed -n "${LINE}p" | awk -F ';' '{print $1}' | sed -e 's/\"/\\\\"/g' -e 's/\//\\\//g' -e 's/^[ \t]*//' -e 's/[ \t]*$//')
		VARIANT_NAME=$(cat /tmp/${FILE_PART}_${i} | sed -n "${LINE}p" | awk -F ';' '{print $2}' | sed -e 's/\"/\\\\"/g' -e 's/\//\\\//g' -e 's/^[ \t]*//' -e 's/[ \t]*$//' \
		-e 's/\"/\\"/g' -e 's/\//\\//g' -e 's/(/\\(/g' -e 's/)/\\)/g' -e 's/\./\\./g')
		VARIANT_VAL=$(cat /tmp/${FILE_PART}_${i} | sed -n "${LINE}p" | awk -F ';' '{print $3}' | sed -e 's/\"/\\\\"/g' -e 's/\//\\\//g' -e 's/^[ \t]*//' -e 's/[ \t]*$//')
...
#	curl -s -X POST  "https://api.moysklad.ru/api/remap/1.2/entity/product" \
